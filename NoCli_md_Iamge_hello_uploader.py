...
def main():
    # 配置
    md_file = r"测试笔记.md"  # 指定Markdown文件路径
    album_id = 1241  # 指定Hello图床相册ID 未选择相册ID为0
    
    try:
        # 处理markdown文件
        logger.info("开始处理markdown文件...")
        new_content = process_markdown(md_file, album_id)
        
        # 保存新文件
        new_file = save_new_markdown(md_file, new_content)
